...
import { ProductSearchRequestModel, ProductSearchResponseModel, PageResultResponseModel, ProductForm } from '../models/product.model';
...
export class ProductService{
    ...
    public createProduct(request: ProductForm, validateHelper?: ErrorEditorState): Observable<any> {
        return this.http.post<any>(this.apiUrl + '/CreateProduct', request).pipe(this.apiPipe(validateHelper));
    }

    public updateProduct(request: ProductForm, validateHelper?: ErrorEditorState): Observable<any> {
        return this.http.put<any>(this.apiUrl + '/UpdateProduct', request).pipe(this.apiPipe(validateHelper));
    }
    // ตัวดักจับ Error 
